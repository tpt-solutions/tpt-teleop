...
//! simulated quadrotor — hostile inputs bounded, fence breaches intercepted,
...
use tpt_teleop_hal::sim::DT_S;
use tpt_teleop_hal::{CanFrame, QuadDrone, World, can_pair, ids};
use tpt_teleop_ring::SpscRing;
use tpt_teleop_safety::{
    GeoFence, KinematicLimits, SafetyConfig, SafetyLoop, axis,
};

/// Builds a loop wired between fresh rings with the given config.
fn make_loop(cfg: SafetyConfig) -> (Arc<SpscRing<ControlCommand>>, Arc<SpscRing<ControlCommand>>, SafetyLoop) {
    ...
    let l = SafetyLoop::new(Arc::clone(&input), Arc::clone(&output), Arc::clone(&machine), cfg);
    (input, output, l)
}

fn teleop_cmd(seq: u64, ts: u64, roll: f32, pitch: f32, throttle: f32, lat_x: f32) -> ControlCommand {
...
fn hostile_operator_commands_emerge_bounded_and_slewed() {
    let cfg = SafetyConfig {
        limits: KinematicLimits { max_axis_step: 0.02, ..KinematicLimits::default() },
    ...
    for i in 0..200u64 {
        input.push(teleop_cmd(i, i * 5_000_000, 9.9, -9.9, 0.7, 5.0)).unwrap();
        let st = l.process_one(i * 5_000_000).expect("command pending");
        let out = output.pop().expect("output produced");
        assert_eq!(st.drained, 1);
        assert!(out.axes[axis::ROLL].abs() <= 0.35 + 1e-6, "tilt clamp violated");
...
fn hard_fence_breach_latches_estop_and_zeroes_output() {
    ...
    l.set_pose(&tpt_teleop_hal::Pose6D { x: 900.0, y: 0.0, z: 40.0, ..Default::default() });

    input.push(teleop_cmd(1, 0, 0.0, 0.0, 0.8, 0.0)).unwrap();
    l.process_one(5_000_000).unwrap();
    ...
    assert!(out.axes.iter().all(|&a| a == 0.0), "E-stop output must be neutral");

    // Sticky: subsequent commands stay overridden even with healthy pose.
    l.set_pose(&tpt_teleop_hal::Pose6D::default());
    input.push(teleop_cmd(2, 0, 0.5, 0.0, 0.8, 0.0)).unwrap();
    l.process_one(10_000_000).unwrap();
    let out = output.pop().unwrap();
    assert_eq!(out.mode(), Some(Mode::EmergencyStop));
}
...
fn manual_emergency_stop_intercepts_immediately() {
    ...
    l.process_one(1_000_000).unwrap();
    let out = output.pop().unwrap();
    assert_eq!(out.mode(), Some(Mode::EmergencyStop));
...
fn handover_blend_is_monotonic_c2_and_reaches_target() {
    let cfg = SafetyConfig { transition_s: 0.1, ..SafetyConfig::default() };
    let (_in, _out, mut l) = make_loop(cfg);

    // Auto → Assist: authority 0 → 0.5 over 100 ms (20 ticks @ 5 ms).
    ...
    for i in 0..40u64 {
        ...
        let a = l.authority();
        if i > 2 {
            assert!(a >= prev - 1e-6, "authority regressed: {prev} → {a}");
        }
        prev = a;
    }
    assert!((l.authority() - 0.5).abs() < 1e-3, "must land exactly on Assist");

    // Assist → FullTeleop continues smoothly to 1.0.
...
fn spawned_safety_thread_processes_until_stopped() {
    let (input, output, l) = make_loop(SafetyConfig {
        transition_s: 0.01,
        ..SafetyConfig::default()
    });
    let _ = l.request_mode(Mode::FullTeleop);
    ...
    for i in 0..50u64 {
        input.push(teleop_cmd(i, i * 1_000_000, 0.1, 0.1, 0.55, 0.0)).unwrap();
...
fn simulated_drone_flies_inside_envelope_under_safety_loop() {
    ...
    let mut imu = tpt_teleop_core::ser::ImuSample::zeroed(0, 0);
    let mut gps = tpt_teleop_core::ser::GpsSample::zeroed(0, 0);
    ...
    for tick in 0..ticks {
        ...
        input.push(teleop_cmd(tick as u64, tick * 5_000_000, 9.0, 0.0, 0.75, 9.0)).unwrap();

        // RT safety intercept.
        l.process_one((tick as u64) * 5_000_000);

        // Flight controller consumes the sanitized command.
        if let Some(mut safe) = output.pop() {
    ...
    // Envelope guarantees: sanitized commands never exceeded the tilt limit…
    assert!(
        max_roll_out <= 0.35 + 1e-5,
        "safety loop leaked an over-limit roll: {max_roll_out}"
    );
    // …and the vehicle stayed inside the physical envelope.
