...
//!
//! Run with: `cargo bench -p tpt-teleop-safety`
...
use tpt_teleop_safety::{GeoFence, SafetyConfig, SafetyLoop};
...
fn main() {
    ...
    let mut loop_ = SafetyLoop::new(
        ...
        Arc::clone(&output),
        Arc::clone(&machine),
        SafetyConfig { transition_s: 0.001, ..SafetyConfig::default() },
    );
    let mut pose = tpt_teleop_hal::Pose6D::default();
    pose.z = 50.0;
    loop_.set_pose(&pose);
    ...
    for i in 0..1000u64 {
        input
            .push(ControlCommand { seq: i, timestamp_ns: i, ..ControlCommand::zeroed(Mode::FullTeleop) })
            .unwrap();
    ...
    println!("p99.9 < 10 µs budget: {}", if under_budget { "PASS" } else { "FAIL" });
}
