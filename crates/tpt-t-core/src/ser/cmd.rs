...
pub struct ControlCommand {
    ...
    pub axes: [f32; 6],
    /// Link-layer CRC placeholder (computed/filled by tpt-t-link).
    pub crc: u32,
}
...
unsafe impl tpt_t_ring::cast::PlainBytes for ControlCommand {}
...
mod tests {
    ...
    fn size_is_exactly_56_no_tail_padding_gaps_unverified_but_castable() {
        ...
        c.seq = 42;
        c.axes[3] = 0.75;
        let bytes = tpt_t_ring::cast::bytes_of(&c);
        assert_eq!(bytes.len(), 56);
        let back: &ControlCommand = tpt_t_ring::cast::ref_from_bytes(bytes).unwrap();
        assert_eq!(back, &c);
