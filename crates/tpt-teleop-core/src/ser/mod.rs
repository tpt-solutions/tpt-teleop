...
//!    included) so it also supports raw casting via `tpt_teleop_ring::cast`
...
unsafe impl tpt_teleop_ring::cast::PlainBytes for WireFrame {}
...
/// payload via `tpt_teleop_ring::cast` — both are zero-copy by construction.
...
mod tests {
    ...
    fn frame_header_cast_roundtrip() {
        ...
        let bytes = tpt_teleop_ring::cast::bytes_of(&f);
        let (back, rest): (&WireFrame, &[u8]) = tpt_teleop_ring::cast::split_prefix(bytes).unwrap();
