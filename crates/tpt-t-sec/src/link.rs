...
const AAD_CONTROL: &[u8] = b"tpt-sec-control";
const AAD_TELEMETRY: &[u8] = b"tpt-sec-telemetry";
...
impl SecureMux {
    ...
    pub fn send_secure_control(
        ...
        self.send_sealed(&scratch[..n], AAD_CONTROL, dst, now_ns)
    }
    ...
    pub fn send_secure_telemetry(
        ...
        self.send_sealed(&scratch[..n], AAD_TELEMETRY, dst, now_ns)
    }
    ...
    pub fn send_secure_raw(
        ...
        self.send_sealed(plaintext, AAD_CONTROL, dst, now_ns)
    }
    ...
    fn send_sealed(
        ...
        let n = self.mux.write_secure_frame(&sealed, &mut buf)?;
    ...
    pub fn recv_decrypt<const N: usize>(
        ...
        match self.mux.recv_frame(rx)? {
            Some(Ok(Inbound::Secure { sealed, .. })) => {
                // Both secured channels share one cipher box; AAD domain
                // separation is enforced on the encrypt side.
                let pushed =
                    decrypt_into_ring(self.session.crypto(), sealed, AAD_CONTROL, ring).is_ok();
...
pub fn decrypt_event_into_ring<const N: usize>(
    ...
    decrypt_into_ring(box_, sealed, AAD_CONTROL, ring)
}
