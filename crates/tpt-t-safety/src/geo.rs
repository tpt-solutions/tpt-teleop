...
mod tests {
    ...
    fn verdict_zones_are_ordered() {
        ...
        assert_eq!(
            fence.evaluate(&pose_at(0.0, 0.0, 50.0)),
            FenceVerdict::Inside
        );
        assert_eq!(
            fence.evaluate(&pose_at(480.0, 0.0, 50.0)),
            FenceVerdict::SoftWarn
        );
        assert_eq!(
            fence.evaluate(&pose_at(501.0, 0.0, 50.0)),
            FenceVerdict::HardBreach
        );
        assert_eq!(
            fence.evaluate(&pose_at(0.0, 0.0, 119.0)),
            FenceVerdict::SoftWarn
        );
        assert_eq!(
            fence.evaluate(&pose_at(0.0, 0.0, 121.0)),
            FenceVerdict::HardBreach
        );
    }
    ...
    fn soft_warning_attenuates_partially_not_fully() {
        ...
        assert!(
            v > 0.2 && v < 0.9,
            "soft warn must partially attenuate, got {v}"
        );
    }
