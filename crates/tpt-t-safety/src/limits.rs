...
pub fn write_emergency_stop(seq: u64, timestamp_ns: u64, cmd: &mut ControlCommand) {
    *cmd = ControlCommand::zeroed(tpt_t_core::Mode::EmergencyStop);
    cmd.seq = seq;
...
mod tests {
    ...
    fn absolute_clamps_engage() {
        ...
        // First tick: slew bounds every limited axis to ±max_axis_step from
        // the previous accepted command (the clamp ceiling is reached
        // gradually — that bounded-acceleration behavior is the point).
        assert!(lim.apply(&prev, &mut cmd));
        assert_eq!(cmd.axes[ROLL], lim.max_axis_step);
        assert_eq!(cmd.axes[PITCH], -lim.max_axis_step);
        assert_eq!(cmd.axes[YAW], lim.max_yaw_norm.min(lim.max_axis_step));

        // Walk forward until the roll reaches the clamp ceiling (epsilon
        // comparison — fp addition isn't bit-exact on the final step).
        let target_roll = 5.0f32;
        let mut settled = false;
        for _ in 0..100 {
            prev = cmd;
            let mut next = ControlCommand::zeroed(Mode::FullTeleop);
            next.axes[ROLL] = target_roll;
            let _ = lim.apply(&prev, &mut next);
            cmd = next;
            if (cmd.axes[ROLL] - lim.max_tilt_norm).abs() < 1e-6 {
                settled = true;
                break;
            }
            assert!(cmd.axes[ROLL] <= lim.max_tilt_norm + 1e-6);
        }
        assert!(settled);
    }
